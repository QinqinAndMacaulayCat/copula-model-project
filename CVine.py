...
class CVine(object):
    layer = {"root": [],  # list of root nodes. ex. in F(u1, u2|v), v is the root node
             "parentnode": {},
             # index of nodes in last level. ex. {1: (1,2)} means the node 1 in this tree level got from the node pair (1,2) in last level
             "node": [],  # index of the nodes. from 0 to l. this is not the actual number index in the original data.
             "pair": [],
             # list of node pairs in the tree, ex. in F(u1, u2|v), (u1, u2) is a node pair. here the node pair is the index of nodes in the root, which is different from the "node".
             "level": 0,  # level of the tree (k). 0-root, 1-1st level, 2-2nd level, ...
             "nodenum": 0,  # number of the nodes in this tree (l). equal to n - k
             "edgenum": 0,
             # number of the edges in this tree. equal to l as our node number is the actual number minus1.
             "V": None,  # h functions in this level. V[:, j] is the h function of node j.
             "dependence": []  # match the pair of nodes
             }

    tree = {"thetaMatrix": None,
            # copula parameter matrix in this level. it is a upper matrix. thetaMatrix[i, j] is the copula parameter in level j between node 1 and node i+1.
    ...
    def __init__(self, U, copulaType="Clayton", dependence=None, max_depth=None):
        ...
        self.U = U
        self.T = U.shape[0]
        self.variable_num = U.shape[
                                1] - 1  # to make the structure more clear, all the variables are indexed from 0. Therefore, when the variable_num is n, we actually have n+1 variables x0, x1, ..., xn.
    ...
    def pair_nodes(self, last_layer):
        ...
        if last_layer[
            "level"] == 0:  # the second layer is not conditional copula, so we just combine the center node with neighor nodes without any condition.
            ...
            common_node = last_pairs[0][
                0]  # always set the first node as the center node in each layer. and the common element between pairs are the left element in the center pair. the common element between the center node and neighbor nodes will be set into the new root.

            new_root = last_layer["root"] + [common_node]
            pair_left = last_pairs[0][
                1]  # the right element in the center pair will be the left element in pairs in this layer.

            for i in range(1, last_layer["nodenum"] + 1):
                pairs.append(tuple((pair_left, last_pairs[i][1])))
                parentnodes[i - 1] = (0,
                                      i)  # the i-1th node in this layer is from the pair (0, i) in last layer. ex. the first layer in the second layer is from the node pair (0, 1) in the first layer.
    ...
    def get_layer_h(self, thetaParams, last_layer):
        ...
        V = np.empty((self.T, last_layer[
            "nodenum"]))  # the total nodes of this layer is the number of nodes in last layer minus 1, which is equal to the edges in last layer.
