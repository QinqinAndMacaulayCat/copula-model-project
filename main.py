...
def main():
    ...
    cv.build_tree()
    cv.fit2()
    simulated_data = cv.simulate(300)
    ...
    cv_clayton.fit2()
    simulated_data_clayton = cv_clayton.simulate(300)
    ...
    dataproc.heatmap(corr_Gaussian, "Correlation_Matrix_of_Gaussian_Copula")
    dataproc.heatmap(corr_Clayton, "Correlation_Matrix_of_Clayton_Copula")
    

    dataproc.plot_kde_comparison(df_u, title="Compare_CDFs_of_Initial_Data")
    dataproc.plot_kde_comparison(df_simuldata_Gaussian, title="Compare_CDFs_of_Gaussian_Copula")
    dataproc.plot_kde_comparison(df_simuldata_Clayton, title="Compare_CDFs_of_Clayton_Copula")
    ...
    with open("result/lower_correlation.txt", "w") as file:
        file.write(f"Initil Data\n")  # Write header
        corr_extreme_init.to_string(file)       # Write DataFrame
        file.write("\n\n")  
        file.write(f"Gaussian Copula\n")
        corr_extreme_Gaussian.to_string(file)# df_simuldata_Gaussian.to_string(file)
        file.write("\n\n")
        file.write(f"Clayton Copula\n")
